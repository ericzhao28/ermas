...
p_ppo = PPO(state_dim, action_dim_p, n_latent_var, lr, betas, gamma, K_epochs,
            eps_clip, 0)
a1_ppo = ERMAS_PPO(state_dim, action_dim_a1, n_latent_var, lr, betas, gamma,
                   K_epochs, eps_clip, 1)
a2_ppo = ERMAS_PPO(state_dim, action_dim_a2, n_latent_var, lr, betas, gamma,
                   K_epochs, eps_clip, 2)
...
for i_episode in range(num_loops):
    ...
    for _ in range(main_episodes):
        ...
        for t in range(max_timesteps):
            ...
            state, rewards, done = env.step([p_action, a1_action, a2_action])
    ...
    for key in new_state_dict:
        old_state_dict[key] = (old_state_dict[key] - beta * a1_lambda *
                               new_state_dict[key]) / (1 + beta * a1_lambda)

    new_state_dict = a2_mppo.policy.state_dict()
    old_state_dict = a2_ppo.policy.state_dict()
    for key in new_state_dict:
        old_state_dict[key] = (old_state_dict[key] - beta * a2_lambda *
                               new_state_dict[key]) / (1 + beta * a2_lambda)
